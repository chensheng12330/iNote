...
//add for sherwin.chen
// 调试日志
-(void) debugLog_c:(const char *)info
{
    NSLog(@"SQLITE INFO:[%@]",[NSString stringWithUTF8String:info]);
}
...
+ (id)databaseWithPath:(NSString*)aPath {
    return FMDBReturnAutoreleased([[self alloc] initWithDBName:aPath]);
}
...
//addtion whith sherwin.chen Gmail:chensheng12330@gmail.com
//say: If it is in the package database, just transferred to document directory.
//
- (id)initWithDBName:(NSString*)_dbName
{
    if (_dbName == nil || [_dbName isEqualToString:@""]) {
        return nil;
    }
    ...
    dbPath = [documentsDirectory stringByAppendingPathComponent:_dbName];
    
    //转换文件到document目录下
    BOOL success;
    NSFileManager *fm = [NSFileManager defaultManager];
    success = [fm fileExistsAtPath:dbPath];  //是否已存在该数据库
    
    // if no
    if (!success) {
        
        NSError *error;
        NSString *defaultDBPath = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:_dbName];
        success = [fm copyItemAtPath:defaultDBPath toPath:dbPath error:&error];  //copy db file from app packges to app doucument
		if(!success) {
            [self debugLog_s:[error localizedDescription]];
            return nil;
		}
    }
