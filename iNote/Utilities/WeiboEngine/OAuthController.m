...
- (NSString *) getUseNameWithWebString:(NSString *)_webstring;
...
- (void) loadView {
	...
	UINavigationItem				*navItem = [[[UINavigationItem alloc] initWithTitle: stTitle] autorelease];
...
- (NSString *) getUseNameWithWebString:(NSString *)_webstring
{
    ...
    if (range.location != NSNotFound) {
        ...
        NSRange getRange;
        getRange.location = range.location;
        getRange.length   = endRange.location -range.location+1;
        _in_usename =[_webstring substringWithRange:getRange];
        
        NSArray* wordsAr = [_in_usename componentsSeparatedByString:@"\""];
        if (wordsAr.count >3) {
            _in_usename = [wordsAr objectAtIndex:2];
        }
        else{
            _in_usename = nil;
        }
...
- (NSString *) locateAuthPinInWebView: (UIWebView *) webView {
    ...
    //////////find USER_ID
    NSRange range = [[webView stringByEvaluatingJavaScriptFromString: @"document.body.innerHTML"] rangeOfString:@"USER_ID"];
    if (range.location != NSNotFound)
    {
        _engine.username = [self getUseNameWithWebString:[webView stringByEvaluatingJavaScriptFromString: @"document.body.innerHTML"]];
    }
